...
ggplot() + 
    geom_polygon(data = dat.gg, aes(x, y, group = id, fill = factor(id)),
                 colour = NA) +
    geom_text(data = labels_cir, aes(x, y, size = points, label = ID),
              color = "#929299", fontface = "bold",
...
ggplot(songs, aes(axis_x, axis_y)) + 
    geom_text(aes(label = nom_ra),
              size = 1.4, hjust = 0,
              color = "#929299", family = "Roboto Condensed") +
    xlim(1, 5) +
    scale_y_continuous(expand = c(0, 1)) +
    theme_void()

patchwork <- p1 / p2 + plot_layout(heights = c(1, 0.7)) + 
    plot_annotation(
    ...
    19 songs by <span style='color:#ffec1b'>**mixed bands or collaborations rappers**</span> between female and male artists",
    caption = "The size of the bubble represents the number of points gained from all songs on the polling list of the BBC\n\nSource: BBC Music \nGraphic: @ysamano28") &
        theme(plot.background = element_rect(fill = "#252a32", color = "#252a32"),
              plot.title = element_text(family = "Roboto Condensed", face = "bold",
                                        size = 30, color = "white",
                                        hjust = 0.5, margin = margin(t = 15, b = 10)),
              plot.subtitle = element_markdown(family = "Roboto Condensed",
                                               size = 13, color = "white", hjust = 0.5,
                                               margin = margin(b = 0)),
              plot.caption = element_text(family = "Roboto Condensed",
                                          size = 6, color = "white", hjust = 0,
                                          margin = margin(t = 10, b = 5, l = 15)))


ggsave("2020/week_16/hip_hop.png", plot = patchwork, height = 12, width = 8, units = "in", dpi = 300)
